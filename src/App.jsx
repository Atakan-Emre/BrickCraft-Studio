...
function ModelImage({id}) {
  const {t}=useLanguage();
  return <img src={assetUrl(`/models/previews/${id}.png?v=2`)} alt={t('{name} tamamlanmış 3B model',{name:t(MODEL_LIST.find(m=>m.id===id).title)})} draggable="false" loading="lazy"/>;
}
function IconButton({
  label,
  children,
